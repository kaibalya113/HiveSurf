...
const CustomContactForm = ({ 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const scriptUrl = FORM_CONFIG.GOOGLE_APPS_SCRIPT_URL;
          ...
          <div>
           ...
           {submitStatus === 'config-error' && (
             <motion.div
               initial={{ opacity: 0, y: -10 }}
               animate={{ opacity: 1, y: 0 }}
               className="flex items-center space-x-2 text-yellow-400 bg-yellow-400/10 border border-yellow-400/20 rounded-lg p-3"
             >
               <AlertCircle size={20} />
               <div>
                 <div className="font-semibold">Form not configured</div>
                 <div className="text-sm">Please follow the setup guide to configure the Google Apps Script.</div>
               </div>
             </motion.div>
           )}

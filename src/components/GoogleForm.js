...
import { googleFormConfig } from '../config/formConfig';
...
const GoogleForm = ({ 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Use configuration from formConfig.js
      const googleFormUrl = googleFormConfig.formUrl;
      
      // Map form data to Google Form field IDs
      const formFields = {
        [googleFormConfig.fieldIds.name]: formData.name,
        [googleFormConfig.fieldIds.email]: formData.email,
        [googleFormConfig.fieldIds.phone]: formData.phone,
        [googleFormConfig.fieldIds.company]: formData.company,
        [googleFormConfig.fieldIds.service]: formData.service,
        [googleFormConfig.fieldIds.message]: formData.message,
        [googleFormConfig.fieldIds.source]: source,
        [googleFormConfig.fieldIds.timestamp]: new Date().toISOString()
      };
      ...
      // Since Google Forms returns a 200 status even on success, we'll simulate a delay
    ...
    <div className={`bg-slate-900/50 backdrop-blur-sm rounded-xl p-6 border border-slate-700/50 ${className}`}>
      <div className="text-center mb-6">
        ...
        <div className="mt-2 text-xs text-blue-400 bg-blue-400/10 border border-blue-400/20 rounded-lg p-2">
          📊 Google Form Integration - Data stored in Google Sheets & emailed to {googleFormConfig.notificationEmail}
        </div>
      </div>
      ...
      {submitStatus === 'success' ? (
        ...
        >
          ...
          <div className="text-xs text-slate-400 bg-slate-800/50 rounded-lg p-3">
            ...
            <div className="mt-2 text-blue-400">
              📧 Email sent to: {googleFormConfig.notificationEmail}
            </div>
          </div>
        </motion.div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="text-center">
            <a 
              href={googleFormConfig.spreadsheetUrl}
              target="_blank" 
              rel="noopener noreferrer"
              className="inline-flex items-center space-x-2 text-xs text-slate-400 hover:text-primary-400 transition-colors"
            >
              <ExternalLink size={12} />
              <span>View Responses in Google Sheets</span>
            </a>
          </div>
        </form>
      )}
